...
process stitch {
    ...
    output:
    path "${base}_${slideID}_${params.z_mode}" into stitched_meas_for_log, stitched_meas_for_rendering

    script:
    base = file(meas).getName()
    """
    acapella -license /home/acapella/AcapellaLicense.txt -s IndexFile="/data_in/$meas/Images/Index.idx.xml" -s OutputDir="./${base}_${slideID}_${params.z_mode}" -s ZProjection="${params.z_mode}" -s OutputFormat=tiff -s Silent=false -s Channels="ALL" -s Gap=${gap} /home/acapella/StitchImage.script
    """
}
