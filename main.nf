...
csv_with_export_paths.splitCsv(header:true, sep:"\t")
    .map{it -> [it.measurement_name, it.Stitching_Z, it.gap, it.SlideID]}
...
process stitch {
    ...
    input:
    tuple val(meas), val(z_mode), val(gap), val(slideID) from stitching_features

    output:
    path "${base}_${slideID}" into stitched_meas_for_log, stitched_meas_for_rendering

    script:
    base = file(meas).getName()
    """
    acapella -license /home/acapella/AcapellaLicense.txt -s IndexFile="/data_in/$meas/Images/Index.idx.xml" -s OutputDir="./${base}_${slideID}" -s ZProjection="${params.z_mode}" -s OutputFormat=tiff -s Silent=false -s Channels="ALL" -s Gap=${gap} /home/acapella/StitchImage.script
    """
}
...
/*process post_process {*/
    ...
    /*output:*/
    /*path "${meas_folder}_*.tsv" into updated_log*/
