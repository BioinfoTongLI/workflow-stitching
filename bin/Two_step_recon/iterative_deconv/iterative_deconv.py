...
def iterative_deconv(data, kernel, iteration, rule):
    ...
    if data.ndim > 2:
        data_de = xp.zeros(
            (data.shape[0], data.shape[1], data.shape[2]), dtype="float32"
        )
        for i in range(0, data.shape[0]):
            data_de[i, :, :] = (
                deblur_core(data[i, :, :], kernel, iteration, rule)
            ).real
    else:
    ...
    if rule == 2:
        ...
        for i in range(0, iteration):
            if i == 0:
                ...
                xk = data + t * xp.fft.ifftn(xp.conj(otf)) * (
                    xp.fft.fftn(data) - (otf * xp.fft.fftn(data))
                )
            else:
                gamma2 = (
                    1 / 2 * (4 * gamma1 * gamma1 + gamma1**4) ** (1 / 2) - gamma1**2
                )
                beta = -gamma2 * (1 - 1 / gamma1)
                yk_update = xk + beta * (xk - xk_update)
                yk = yk_update + t * xp.fft.ifftn(
                    xp.conj(otf) * (xp.fft.fftn(data) - (otf * xp.fft.fftn(yk_update)))
                )
        ...
        for iter in range(0, iteration):
            ...
            xk = (
                yk
                * ((xp.fft.ifftn(xp.conj(otf) * rliter1)).real)
                / ((xp.fft.ifftn(xp.fft.fftn(xp.ones(data.shape)) * otf)).real)
            )
            ...
            if iter == 0:
                ...
                alpha = sum(sum(vk_update * vk)) / (
                    sum(sum(vk_update * vk_update)) + 1e-10
                )
...
def psf2otf(psf, outSize):
    ...
    if (
        xp.max(xp.abs(xp.imag(otf))) / xp.max(xp.abs(otf))
        <= nOps * xp.finfo(xp.float32).eps
    ):
        otf = xp.real(otf)
    return otf
