...
def main(evaluation_input_data_file="index.xml", images_path="."):
    ...
    NS = {"PE": pe_ns, "OME": "http://www.openmicroscopy.org/Schemas/OME/2016-06"}
    ...
    for row in range(plate_rows):
        for column in range(plate_columns):
            ...
            image_psx = (
                float(pixel_data["PhysicalSizeX"])
                if "PhysicalSizeX" in pixel_data
                else None
            )
            image_psy = (
                float(pixel_data["PhysicalSizeY"])
                if "PhysicalSizeY" in pixel_data
                else None
            )
            image_psz = (
                float(pixel_data["PhysicalSizeZ"])
                if "PhysicalSizeZ" in pixel_data
                else None
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Build OME-XML from Perkin Elmer index.xml output of stitching"
    )
    parser.add_argument(
        "--evaluation_input_data_file",
        type=str,
        default="index.xml",
        required=False,
